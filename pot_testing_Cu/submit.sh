...
if [[ ${pot} = "ace" ]]; then
	pstyle="pace"
	pcoeff="../../potential/${potfilename} Cu"
elif [[ ${pot} = "eam" ]]; then
	pstyle="eam"
...
# Phonon
cd ../PHONON
cp ../RELAX/relax_Cu.data .
phonopy --symmetry relax_Cu.data
phonopy --lammps -c Punitcell -d --dim 3 3 3
...
for i in $(seq 2 8)
 do
    mpirun -n 28  ${LMMP} -in in.melting -v epoch ${i}
done
